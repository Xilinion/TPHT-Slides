...
\begin{frame}{Chained-TPHT}
  ...
  \begin{tikzpicture}[x=1cm,y=1cm,>=Latex,every node/.style={font=\small}]
    \node[draw=CornellRed,fill=CornellRed!7,rounded corners=2pt,line width=1.0pt,
          minimum width=2.25cm,minimum height=0.55cm,font=\normalsize\bfseries] (insert) at (-4.80,2.55)
          {\apiop{Insert}{k,v}};

    \node[draw=Ink,fill=white,line width=0.9pt,rounded corners=3pt,
          minimum width=2.25cm,minimum height=0.55cm,font=\normalsize\bfseries] (split) at (-2.38,2.55)
          {split \(k=q\circ r\)};
    ...
    \node[anchor=north,font=\scriptsize,text=CornellRed] at (-1.02,2.27) {bucket id};
    \node[anchor=north,font=\scriptsize,text=BlueGray] at (0.10,2.27) {stored key bits};
    ...
    \node[anchor=north west,font=\scriptsize,text=BlueGray] at (-5.72,1.08) {one tiny pointer per quotient group};
    \foreach \idx/\val/\y in {0/\(p_0\)/0.58,1/\(p_1\)/0.19,q/\(p_q\)/-0.20,\ldots/\(\cdots\)/-0.59} {
      ...
      \node[font=\scriptsize,text=BlueGray] at (-5.18,\y) {\(\idx\)};
      \node[font=\scriptsize\bfseries] at (-4.20,\y) {\val};
    }
    \draw[CornellRed,line width=1.15pt,rounded corners=1pt] (-5.45,-0.35) rectangle (-3.05,-0.05);
    \node[font=\scriptsize,text=CornellRed,anchor=east] (owner) at (-5.54,-0.20) {owner \(A_q\)};
    ...
    \node[anchor=north west,font=\scriptsize,text=BlueGray] at (-1.35,1.08) {all chain nodes live in one compact pool};
    ...
    \foreach \x/\lab in {-0.80/\(\cdots\),0.05/\(\mathrm{bin}\),0.90/\(\cdots\)} {
      ...
      \node[font=\scriptsize,text=BlueGray,rotate=90] at (\x,0.10) {\lab};
    }
    ...
    \node[anchor=south,font=\scriptsize\bfseries,text=CornellRed] at (0.05,0.83) {decoded slot};
    \draw[-Latex,CornellRed,very thick] (headslot) to[out=-10,in=170]
      node[pos=0.55,fill=white,inner sep=1.3pt,font=\scriptsize\bfseries,text=CornellRed]
      {TinyDeref\((A_q,p_q)\)} (slot);

    \node[draw=Ink,fill=white,line width=0.9pt,rounded corners=2pt,
          minimum width=1.56cm,minimum height=0.48cm,font=\scriptsize\bfseries] (oldnode) at (2.20,0.10)
          {\(r_\ell,v_\ell\;|\;p_\ell\)};
    \node[draw=CornellRed,fill=CornellRed!7,line width=1.0pt,rounded corners=2pt,
          minimum width=1.42cm,minimum height=0.48cm,font=\scriptsize\bfseries,text=CornellRed] (newnode) at (4.08,0.10)
          {\(r,v\;|\;\bot\)};
    \draw[-Latex,Ink,thick] (slot) -- (oldnode.west);
    \draw[-Latex,CornellRed,thick] (oldnode.east) -- (newnode.west);
    \node[anchor=south,font=\scriptsize\bfseries,text=BlueGray] at (3.08,0.50)
