...
\begin{frame}{Ingredient 2: Quotienting}
  ...
  \begin{tikzpicture}[x=1cm,y=1cm,every node/.style={font=\normalsize},
    ...
    \draw[fieldbrace,CornellRed] (-5.60,2.24) -- (-4.66,2.24)
      node[midway,above=8pt,xshift=-0.45cm,font=\normalsize\bfseries,text height=1.6ex,text depth=0.5ex,text=CornellRed] {quotient \(q\)};
    \draw[fieldbrace,Purple] (-4.62,2.24) -- (-2.89,2.24)
      node[midway,above=8pt,xshift=0.25cm,font=\normalsize\bfseries,text height=1.6ex,text depth=0.5ex,text=Purple] {remainder \(r\)};
...
\begin{frame}{Chained-TPHT}
  ...
  \vspace{0.06cm}
  \begin{center}
  \begin{tikzpicture}[x=1cm,y=1cm,>=Latex,every node/.style={font=\normalsize}]
    \node[draw=CornellRed,fill=CornellRed!7,rounded corners=2pt,line width=1.0pt,
          minimum width=2.10cm,minimum height=0.50cm,font=\normalsize\bfseries] (insert) at (-5.10,2.55)
          {\apiop{Insert}{k,v}};

    \node[draw=Ink,fill=white,line width=0.9pt,rounded corners=3pt,
          minimum width=2.15cm,minimum height=0.50cm,font=\normalsize\bfseries] (split) at (-2.92,2.55)
          {split \(k=q\circ r\)};
    \draw[-Latex,thick] (insert.east) -- (split.west);

    \node[draw=CornellRed,fill=CornellRed!9,line width=1.0pt,rounded corners=2pt,
          minimum width=0.70cm,minimum height=0.44cm,font=\large\bfseries,text=CornellRed] (qbits) at (-1.45,2.55) {\(q\)};
    \node[draw=BlueGray,fill=BlueGray!8,line width=1.0pt,rounded corners=2pt,
          minimum width=1.05cm,minimum height=0.44cm,font=\large\bfseries,text=BlueGray] (rbits) at (0.02,2.55) {\(r\)};
    \draw[-Latex,thick] (split.east) -- (qbits.west);
    \node[anchor=north,font=\tiny,text=CornellRed] at (-1.18,2.27) {bucket id};
    \node[anchor=north,font=\scriptsize,text=BlueGray] at (0.02,2.27) {stored key bits};

    \draw[Ink,line width=0.9pt,rounded corners=4pt,fill=CornellGray!55] (-5.92,-1.42) rectangle (-2.10,1.50);
    \node[anchor=north west,font=\large\bfseries] at (-5.72,1.34) {head array};
    \node[anchor=north west,font=\tiny,text=BlueGray] at (-5.72,0.94) {one tiny pointer per quotient group};
    \foreach \idx/\val/\y in {0/\(p_0\)/0.39,1/\(p_1\)/0.03,q/\(p_q\)/-0.33,\ldots/\(\cdots\)/-0.69} {
    ...
    \draw[CornellRed,line width=1.15pt,rounded corners=1pt] (-5.45,-0.48) rectangle (-3.05,-0.18);
    \node[font=\small,text=CornellRed,anchor=west] (owner) at (-5.72,-1.14) {owner \(A_q\)};
    \coordinate (headslot) at (-4.20,-0.33);
    \draw[-Latex,CornellRed,very thick,rounded corners=4pt] (qbits.south)
      -- (-1.81,2.05) -- (-1.81,-0.33) -- (headslot);

    \draw[Ink,line width=0.9pt,rounded corners=4pt,fill=CornellGray!55] (-1.52,-1.42) rectangle (5.82,1.50);
    \node[anchor=north west,font=\large\bfseries] at (-1.30,1.34) {dereference table};
    \node[anchor=north west,font=\scriptsize,text=BlueGray] at (-1.30,0.94) {all chain nodes live in one compact pool};

    \foreach \x/\lab in {-0.80/\(\cdots\),0.05/\(\mathrm{bin}\),0.90/\(\cdots\)} {
      \draw[Ink,line width=0.65pt,rounded corners=1pt,fill=white] (\x-0.30,-1.08) rectangle (\x+0.30,0.30);
      \node[font=\small,text=BlueGray,rotate=90] at (\x,-0.40) {\lab};
    }
    \draw[CornellRed,line width=1.15pt,rounded corners=1pt,fill=white] (-0.25,-1.08) rectangle (0.35,0.30);
    \coordinate (slot) at (0.05,-0.40);
    \fill[CornellRed] (slot) circle (3.0pt);
    \node[anchor=south,font=\scriptsize\bfseries,text=CornellRed] at (0.05,0.34) {decoded slot};
    \draw[-Latex,CornellRed,very thick] (headslot) to[out=-10,in=170] (slot);
    \node[font=\scriptsize\bfseries,text=CornellRed] at (-3.20,-1.15)
      {TinyDeref\((A_q,p_q)\)};

    \node[draw=Ink,fill=white,line width=0.9pt,rounded corners=2pt,
          minimum width=1.56cm,minimum height=0.48cm,font=\small\bfseries] (oldnode) at (2.20,-0.40)
          {\(r_\ell,v_\ell\;|\;p_\ell\)};
    \node[draw=CornellRed,fill=CornellRed!7,line width=1.0pt,rounded corners=2pt,
          minimum width=1.42cm,minimum height=0.48cm,font=\small\bfseries,text=CornellRed] (newnode) at (4.08,-0.40)
          {\(r,v\;|\;\bot\)};
    \draw[-Latex,Ink,thick] (slot) -- (oldnode.west);
    \draw[-Latex,CornellRed,thick] (oldnode.east) -- (newnode.west);
    \node[anchor=south,font=\small\bfseries,text=BlueGray] at (3.08,-0.06)
      {chain for quotient \(q\)};

    \draw[-Latex,BlueGray,very thick] (rbits.south)
      .. controls (5.28,2.08) and (5.25,0.32) .. (newnode.north);
    \node[draw=Gold,fill=Gold!20,line width=1.0pt,rounded corners=3pt,
          inner sep=3.5pt,font=\small\bfseries,rotate=-5] at (4.26,-1.08)
          {store \(r\), not \(q\)};
  \end{tikzpicture}
  \end{center}
