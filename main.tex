...
\begin{frame}{Transition: Experimental Results}
  ...
  \begin{center}
    ...
    \vspace{0.08cm}
    {\small\color{BlueGray} We evaluate both TPHT variants under the same conditions as strong production-style hash tables.}
  \end{center}
  ...
  \vspace{0.06cm}
  \centering
  {\small\textbf{Space efficiency} = raw key--value payload / maximum allocated memory footprint.}
\end{frame}
...
\begin{frame}{Result: TPHT Moves the Space Frontier}
    ...
    \column{0.52\textwidth}
      ...
      \begin{tikzpicture}
        \begin{axis}[
          ...
          legend style={draw=none,fill=none,at={(0.50,1.03)},anchor=south,legend columns=2,font=\scriptsize},
          bar width=4.3pt,
  ...
  \vspace{0.02cm}
  \centering
  {\small Chained-TPHT goes beyond the raw key-value payload size; Flattened-TPHT is both compact and fast.}
\end{frame}
...
\begin{frame}{Figure 8(a): Insertion Throughput vs. Space}
  ...
  \begin{tikzpicture}
    \begin{axis}[
      ...
      major grid style={line width=0.25pt,draw=CornellGray},
      minor tick num=1,
      legend style={draw=none,fill=none,at={(0.5,1.04)},anchor=south,legend columns=3,font=\scriptsize},
      ...
      \node[anchor=south east,font=\scriptsize,text=CornellRed] at (axis cs:99.2,0.6) {100\% payload};
  ...
  \vspace{-0.02cm}
  \centering
  {\small Each curve sweeps load factor; TPHT pushes farther right while keeping competitive insertion throughput.}
\end{frame}
