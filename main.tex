...
\begin{frame}{Tiny Pointer: Implementation}
  ...
  \begin{tikzpicture}[x=1cm,y=1cm,>=Latex,every node/.style={font=\normalsize}]
    \node[font=\normalsize\bfseries] (api) at (-4.05,3.48)
      {\apiop{Alloc}{\text{A}, \text{B}} \(\to p =\)};
    \node[anchor=west,align=left,font=\small,BlueGray] at (3.62,3.50)
      {\(\text{A}\): owner\\\(\text{B}\): object};
    \node[font=\normalsize\bfseries,text=CornellRed] (dirbit) at (-2.35,3.48) {\(0b1\)};
    \node[font=\normalsize\bfseries] (concat) at (-1.86,3.48) {\(\circ\)};
    \node[font=\normalsize\bfseries,text=CornellRed] (offsetbits) at (-0.82,3.48) {\(0b00000010\)};
    ...
    \node[fill=white,inner sep=1pt,font=\scriptsize\bfseries,text=CornellRed] at (0.58,2.20) {7-bit offset};
    \draw[-Latex,CornellRed,thick] (h1.north) to[out=95,in=-80] (dirbit.south);
    \node[fill=white,inner sep=1pt,font=\scriptsize\bfseries,text=CornellRed] at (-1.52,3.02) {direction bit = 1};
    ...
    \node[draw=Gold,fill=Gold!22,line width=2.0pt,rounded corners=6pt,
          inner xsep=10pt,inner ysep=5pt,font=\large\bfseries,text=Ink,rotate=4]
          at (4.55,-0.38) {Magic!};
  \end{tikzpicture}
  \end{center}
  \vspace{-0.16cm}
...
\begin{frame}{Ingredient 2: Quotienting}
  ...
  \begin{tikzpicture}[x=1cm,y=1cm,>=Latex,every node/.style={font=\normalsize}]
    ...
    \node[text=CornellRed,font=\large\bfseries] (qlabel) at (-5.50,3.08) {quotient \(q\)};
    \node[text=BlueGray,font=\large\bfseries] (rlabel) at (-3.30,3.08) {remainder \(r\)};
